...
v2.1 支持两种来源，并使用同一套金额校验、AI 审查、区域分配和报告口径：
...
1. 会员模式先解析公司合法导出的两份表，再按标题、项目编号、招标人和日期与四川政府公开公告做高置信反查；只有唯一命中的官方页面才会补入正文，反查失败不会阻断会员表日常处理。
2. 官方免费模式先做标题召回和结果类过滤，再只为候选读取官网完整正文。
3. Python 规则执行保险目标险种预筛、工程 `1000万元` 门槛、异常金额保护和跨关键词公告去重。
4. `deepseek-v4-flash` 批量核验保险险种与工程语义；会员工程导出表中金额正常且达到门槛的记录按部门原有口径完整保留，AI 负责分类和证据说明，不再静默删项。
5. 官方正文只截取与险种、工程性质和金额有关的有界片段；联系人、电话、邮箱和链接不发送给模型。
6. 正式输出包括老师版 5 列汇总表、精简业务明细、群发文案、成都保险汇总图和“一项目一张”高清重点卡 ZIP；内部审计页默认隐藏。

系统会生成可复制的企业微信文案、成都保险汇总 PNG、一项目一图 ZIP 和专业 Excel，全程使用内存文件流。
